...
namespace TAK_CameraController
{
	...
	public class CameraController : MonoBehaviour
	{
        ...
        [SerializeField] bool useIsPointerOverGameObject = false;
		...
		void mouseDragEvent(Vector3 mousePos)
		{
			...
			else if (Input.GetMouseButton((int)MouseButtonDown.MBD_MIDDLE))
			{
				...
				{
					this.cameraTranslate(new Vector3(diff.x / 5.0f, -diff.y / 5.0f, diff.z / 5.0f));
				}
				else
				{
					this.cameraTranslate(new Vector3(diff.x / 10.0f, -diff.y / 10.0f, diff.z / 10.0f));
				}
			...
			else if (Input.GetMouseButton((int)MouseButtonDown.MBD_RIGHT))
			{
				...
				this.cameraRotate(new Vector3(diff.y, diff.x, 0.0f) / 2.0f);
			}
